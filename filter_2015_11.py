...
def time_parse(id,time):
	'''if not time:
		print(id)
		return datetime.strptime('10/10/2010 10:10:10', '%m/%d/%Y %H:%M:%S')'''
...
with open(inputFile, 'r') as unfilteredList:
    ...
    for job in csvReader:
        if (job['queue'] in availableQueues) and (job['jobevent'] == 'E' and job['start'] and job['end']):      # filter out successfully exited jobs with Exit_status of 0
            ...
            while tempNodeStartIndex != -1:
                ...
                jobStartTime = time_parse(job['jobID'], job['start'])
                jobEndTime = time_parse(job['jobID'], job['end'])
